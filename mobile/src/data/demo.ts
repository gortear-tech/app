...
export function getPhotosForPage(pageId: string): Photo[] {
  return demoPhotos.filter((photo) => photo.pageId === pageId);
}

export function getCalendarForPage(pageId: string): CalendarItem[] {
  return demoCalendar.filter((item) => item.pageId === pageId);
}

export function getStyleHistoryForPage(pageId: string): StyleHistoryEntry[] {
